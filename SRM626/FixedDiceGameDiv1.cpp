...
class FixedDiceGameDiv1 {
	...
	double getExpectation(int a, int b, int c, int d) {
		...
		for (int i = 0; i < a-1; i++) {
			...
			for (int j = 0; j <= 2500; j++) A[j] = tA[j];
		}

		for (int i = 1; i <= d; i++) B[i]++;
		for (int i = 0; i < c-1; i++) {
			double tB[2501] = {0};
			for (int j = 1; j <= d; j++) {
				for (int k = 0; k < 2500; k++) {
			...
			for (int j = 0; j <= 2500; j++) B[j] = tB[j];
		}

		for (int i = 0; i < d*c; i++) B[i+1] += B[i];

		double sum = 0.0, total = 0.0;
		for (int i = 1; i <= 2500; i++) {
			sum += i*A[i]*B[min(i-1,c*d)];
			total += A[i]*B[min(i-1,c*d)];
		}

//		for (int i = 0; i < 2500; i++) cout << i << " " << A[i] << endl;
